...
def process_raw_eeg_message(channel, method, properties, body, db_conn):
    ...
    try:
        ...
        latest_esp_micros = struct.unpack_from('<I', raw_bytes, (num_samples - 1) * ESP32_SENSOR_SIZE + (ESP32_SENSOR_SIZE - 4))[0]
        ...
        for i in range(num_samples):
            ...
            eeg_values, accel_values, gyro_values, trigger, impedance_values, esp_micros = \
                list(unpacked_data[0:8]), list(unpacked_data[8:11]), list(unpacked_data[11:14]), \
                unpacked_data[14], list(unpacked_data[15:23]), unpacked_data[23]

            record_timestamp = esp_boot_time_server + timedelta(microseconds=esp_micros)

            # レコードに実験IDを追加
            eeg_records.append((record_timestamp, device_id, active_experiment_id, eeg_values, impedance_values, trigger))
            imu_records.append((record_timestamp, device_id, active_experiment_id, accel_values, gyro_values))
        ...
        processed_message = {
            "device_id": device_id, "eeg_data": [rec[3] for rec in eeg_records]
        }
        channel.basic_publish(exchange='processed_data_exchange', routing_key='eeg.processed', body=json.dumps(processed_message))
...
def process_media_message(channel, method, properties, body, db_conn):
    ...
    try:
        ...
        device_id = message['device_id']
        active_experiment_id = get_active_experiment_id(db_conn, device_id)
        timestamp = datetime.fromtimestamp(message['timestamp_ms'] / 1000.0, tz=timezone.utc)

        with db_conn.cursor() as cur:
            cur.execute(
                "INSERT INTO media_files (timestamp, device_id, epoch_id, experiment_id, image_data, audio_data, metadata) VALUES (%s, %s, %s, %s, %s, %s, %s)",
                (
                    timestamp, device_id, message['epoch_id'], active_experiment_id,
                    ...
                    json.dumps({
                        "image_mimetype": message['image']['mimetype'] if message.get('image') else None,
                        "audio_mimetype": message['audio']['mimetype'] if message.get('audio') else None
                    })
                )
            )
            db_conn.commit()
...
def main():
    ...
    channel.exchange_declare(exchange='processed_data_exchange', exchange_type='topic', durable=True)
