...
async def generate_ai_summary(recommendations: List[dict]) -> str:
    ...
    if not recommendations:
        return "ユーザーが高い関心を示した特定の製品は見つかりませんでした。キャリブレーションデータや計測条件をご確認ください。"
    ...
    try:
        ...
        product_descriptions = [
            " ".join(filter(None, [item.get('brand_name'), item.get('item_name'), f"({item.get('file_name')})"]))
        ...
        return f"解析の結果、{len(recommendations)}件の製品に高い関心が示されました。(AIサマリーの生成中にエラー発生)"
...
async def _load_experiment_metadata(experiment_id: UUID) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    def _query() -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        stimuli_columns = ['stimulus_id', 'file_name', 'trial_type', 'item_name', 'brand_name', 'description', 'category', 'gender']
        stimuli_df = (
...
async def run_full_analysis(
    ...
    if sessions_df.empty:
        raise HTTPException(status_code=404, detail="No completed sessions with valid events found for this experiment.")
    ...
    if cal_epochs is None or len(cal_epochs) == 0:
        raise HTTPException(status_code=404, detail="Could not create epochs from calibration BIDS data. Check if event data exists and is correctly formatted.")
    if main_epochs is None or len(main_epochs) == 0:
        raise HTTPException(status_code=404, detail="Could not create epochs from main task BIDS data. Check if event data exists and is correctly formatted.")
    ...
    if main_epochs.metadata is None:
        raise HTTPException(status_code=500, detail="Epoch metadata is missing for main task sessions.")
