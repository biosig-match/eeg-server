  ...
  while True:
    ...
    for user_id, buffer in current_buffers.items():
      ...
      try:
        ...
        fig_coh, _ = plot_connectivity_circle(con_matrix, CHANNEL_NAMES, show=False, vmin=0, vmax=1)
      ...
      def callback(ch, method, properties, body):
        try:
          ...
          for idx in range(num_points):
            ...
            eeg_point = struct.unpack_from("<" + "H" * NUM_EEG_CHANNELS, decompressed, offset)
          ...
          print(f"RabbitMQコールバックでZstandard解凍エラー: {exc}。送信データ形式を確認してください。")
          ch.basic_ack(delivery_tag=method.delivery_tag)
        except Exception as exc:  # pragma: no cover - logging purpose only
