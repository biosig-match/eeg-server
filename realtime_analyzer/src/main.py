...
# b: int8 (インピーダンス x8)
# I: uint32 (タイムスタンプ)
ESP32_SENSOR_FORMAT = '<' + 'H'*NUM_EEG_CHANNELS + 'f'*3 + 'f'*3 + 'B' + 'b'*NUM_EEG_CHANNELS + 'I'
...
try:
    ...
    print(f"警告: 電極位置(Montage)の設定に失敗しました。空間的な色のプロットが機能しない可能性があります。エラー: {e}")
...
def analysis_worker():
    ...
    while True:
        ...
        try:
            ...
            fig_psd = raw.compute_psd(fmin=1, fmax=45, n_fft=SAMPLE_RATE, verbose=False).plot(show=False, spatial_colors=True)
            psd_b64 = fig_to_base64(fig_psd)

            # 3. α帯 (8-13Hz) のコヒーレンス (同期度) の計算とプロット
            epochs = mne.make_fixed_length_epochs(raw, duration=ANALYSIS_WINDOW_SEC, preload=True, verbose=False)
            con = spectral_connectivity_epochs(epochs, method="coh", sfreq=SAMPLE_RATE, fmin=8, fmax=13, faverage=True, verbose=False)
            con_matrix = con.get_data(output="dense")[0]
            fig_coh, _ = mne.viz.plot_connectivity_circle(con_matrix, CHANNEL_NAMES, show=False, vmin=0, vmax=1)
            ...
            print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] リアルタイム解析結果を更新しました。")
        except Exception as e:
...
def rabbitmq_consumer():
    ...
    while True:
        try:
            ...
            channel.queue_bind(exchange='processed_data_exchange', queue=queue_name, routing_key='eeg.processed')
            ...
            print(f"予期せぬエラーでコンシューマスレッドが停止しました: {e}。5秒後に再起動します...")
            time.sleep(5)
