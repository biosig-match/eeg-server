...
import { debugLog, warnLog, errorLog } from '../../infrastructure/logger';
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 404 || response.status === 403) {
          const errorBody = await parseJsonValue(response, {
            error: 'Authorization failed.',
          });
          return c.json(errorBody, response.status as 403 | 404);
        }
      ...
      if (!authPayload || typeof authPayload.authorized !== 'boolean') {
        throw new Error('Auth service returned an unexpected payload.');
      }

      const { authorized } = authPayload;
