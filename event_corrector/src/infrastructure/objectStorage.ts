...
export async function ensureObjectStorageBucket(
  ...
  await ensureBucketExists(objectStorageClient, config.OBJECT_STORAGE_RAW_DATA_BUCKET, {
    ...
    onCreateStart: () =>
      console.warn(
        `[ObjectStorage] Bucket "${config.OBJECT_STORAGE_RAW_DATA_BUCKET}" does not exist. Creating...`,
      ),
    onCreateSuccess: () =>
      console.log(
        `✅ [ObjectStorage] Bucket "${config.OBJECT_STORAGE_RAW_DATA_BUCKET}" created successfully.`,
      ),
    onAlreadyExists: () =>
      console.log(`✅ [ObjectStorage] Bucket "${config.OBJECT_STORAGE_RAW_DATA_BUCKET}" is ready.`),
    onRetry: ({ attempt, maxAttempts: max, error }) =>
      console.warn(
