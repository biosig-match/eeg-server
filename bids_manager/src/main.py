...
def run_bids_export_task(task_id, experiment_id):
    ...
    try:
        ...
        with db_conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            if not exp_info:
                raise ValueError("指定された実験IDが見つかりません。")

            export_tasks[task_id]["progress"] = 10
            export_tasks[task_id]["message"] = "脳波データを取得しています..."
            cur.execute(
                "SELECT timestamp, eeg_values FROM eeg_raw_data WHERE experiment_id = %s ORDER BY timestamp ASC",
                (experiment_id,),
            )
            eeg_rows = cur.fetchall()
            if not eeg_rows:
                raise ValueError("指定された期間の脳波データが見つかりません。")
            ...
            data_in_microvolts = (eeg_data.astype(np.float64) - 2048.0) * (
                exp_info["metadata"].get("adc_gain", 200.0) / 2048.0
            )
            data_in_volts = data_in_microvolts.T * 1e-6

            info = mne.create_info(
                ch_names=exp_info["metadata"]["channel_names"], sfreq=sample_rate, ch_types="eeg"
            )
            ...
            cur.execute(
                "SELECT * FROM experiment_events WHERE experiment_id = %s ORDER BY onset ASC",
                (experiment_id,),
            )
            ...
            write_raw_bids(
                raw, bids_path, allow_preload=True, format="EDF", overwrite=True, verbose=False
            )
        ...
        export_tasks[task_id] = {
            "status": "completed",
            "progress": 100,
            "message": f"エクスポートが完了しました。ファイルは {bids_path.basename} として保存されました。",
        }
...
def create_experiment():
    ...
    with db_conn.cursor() as cur:
        cur.execute(
            "INSERT INTO experiments (experiment_id, participant_id, device_id, start_time, metadata) VALUES (%s, %s, %s, %s, %s)",
            (
                exp_id,
                data["participant_id"],
                data["device_id"],
...
def upload_events(experiment_id):
    ...
    with db_conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
        ...
        exp_info = cur.fetchone()
        if not exp_info:
            return jsonify({"error": "指定された実験IDが見つかりません。"}), 404

        cur.execute(
            "SELECT MIN(timestamp) as first_sample_time FROM eeg_raw_data WHERE experiment_id = %s",
            (experiment_id,),
        )
        ...
        for index, row in events_df.iterrows():
            ...
            event_records.append(
                (experiment_id, onset_seconds, duration, description, row["image"])
            )
        ...
        if event_records:
            psycopg2.extras.execute_values(
                cur,
                "INSERT INTO experiment_events (experiment_id, onset, duration, description, stimulus_file) VALUES %s",
                event_records,
            )

        # 6. 実験を終了状態にする
        cur.execute(
            "UPDATE experiments SET end_time = %s WHERE experiment_id = %s",
    ...
    return jsonify(
        {
            "status": "events registered and experiment finished",
            "events_matched": len(event_records),
        }
    ), 200
...
def get_export_status(task_id):
    ...
    if not task:
        return jsonify({"error": "タスクIDが見つかりません"}), 404
    return jsonify(task)
