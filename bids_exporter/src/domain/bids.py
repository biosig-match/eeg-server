    ...
    try:
        ...
        with get_db_connection() as conn:
            with get_db_cursor(conn) as cur:
                ...
                # --- 4. 各セッションを処理 ---
                def decompress_payload(payload: bytes) -> bytes | None:
                    attempts: list[dict[str, object]] = [
                        {},
                        {"format": zstd.FORMAT_ZSTD_MAGICLESS},
                    ]

                    for attempt in attempts:
                        try:
                            dctx = zstd.ZstdDecompressor(**attempt)  # type: ignore[arg-type]
                            return dctx.decompress(payload, max_output_size=64 * 1024 * 1024)
                        except zstd.ZstdError:
                            try:
                                with dctx.stream_reader(io.BytesIO(payload)) as reader:  # type: ignore[name-defined]
                                    return reader.read()
                            except zstd.ZstdError:
                                continue
                    return None
                for i, session in enumerate(sessions):
                    ...
                    for obj in data_objects:
                        ...
                        compression_meta = (
                            response.headers.get('x-amz-meta-x-compression', '').lower()
                            if hasattr(response, 'headers')
                            else ''
                        )
                        payload = response.read()

                        if compression_meta == 'none':
                            eeg_payload = payload
                        else:
                            decompressed_data = decompress_payload(payload)
                            if decompressed_data is None:
                                print(
                                    f"Warning: Failed to decompress object {obj['object_id']}. Skipping.",
                                )
                                continue
                            eeg_payload = decompressed_data

                        eeg_data = parse_eeg_data_from_binary(eeg_payload)
                        if eeg_data.size > 0:
