...
def generate_esp32_packet(start_micros, trigger_now=False):
    ...
    for i in range(SAMPLES_PER_PACKET):
        ...
        raw_bytes += struct.pack(ESP32_SENSOR_FORMAT, *eeg, *accel, *gyro, trigger, *impedance, timestamp)
    return raw_bytes
...
def send_eeg_data(ws, experiment_start_time):
    ...
    while not stop_sending_eeg.is_set():
        ...
        if time.time() > experiment_start_time and (time.time() - last_stimulus_time) >= STIMULUS_INTERVAL_SEC:
...
def run_experiment_workflow():
    ...
    print("\n[ステップ1] 新しい実験を開始します...")
    exp_metadata = { "task_name": "simulatederp", "sampling_rate": SAMPLE_RATE, "channel_names": ["Fp1", "Fp2", "F7", "F8", "T7", "T8", "P7", "P8"] }
    response = requests.post(f"{API_BASE_URL}/experiments", json={ "participant_id": PARTICIPANT_ID, "device_id": ESP32_DEVICE_ID, "metadata": exp_metadata })
    if response.status_code != 201:
        print(f"実験の開始に失敗しました: {response.text}"); return
    ...
    if response.status_code != 200:
        print(f"イベントのアップロードに失敗しました: {response.text}"); return
    ...
    if response.status_code != 202:
        print(f"BIDSエクスポートの要求に失敗しました: {response.text}"); return
    ...
    for _ in range(10):
        ...
        if status.get('status') in ['completed', 'failed']: break
