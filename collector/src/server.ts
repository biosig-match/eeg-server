    ...
    ws.on('message', (message) => {
        ...
        if (amqpChannel) {
            try {
                ...
                amqpChannel.publish(RAW_DATA_EXCHANGE, 'eeg.raw', Buffer.from(JSON.stringify(parsedMessage)), { persistent: true });
            } catch (e) {
...
app.post('/api/v1/media', upload.fields([{ name: 'image', maxCount: 1 }, { name: 'audio', maxCount: 1 }]), (req, res) => {
    if (!amqpChannel) {
    ...
    amqpChannel.publish(RAW_DATA_EXCHANGE, 'media.raw', Buffer.from(JSON.stringify(message)), { persistent: true });
