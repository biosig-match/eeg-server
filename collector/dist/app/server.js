...
app.post('/api/v1/data', zValidator('json', dataSchema), async (c) => {
    ...
    console.log(`[HTTP:/data] Published sensor data for user: ${user_id} (${binaryPayload.byteLength} bytes)`);
...
app.post('/api/v1/media', async (c) => {
    ...
    console.log(`[HTTP:/media] Queued media file for user: ${metadata.user_id}, session: ${metadata.session_id} (${fileBuffer.byteLength} bytes)`);
    return c.json({ status: 'accepted' }, 202);
});
app.onError((err, c) => {
